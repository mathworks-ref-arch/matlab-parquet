classdef (Abstract = true) Core < handle
    ...
    methods
        ...
        function out = getSourceFolder(obj,varargin)
            ...
            % folder.
            %
            % Example: Return the full path to Java\target\*.jar
            %
            %    obj.getSourceFolder('Java','target','myjar.jar')
            %
            %    PROJECT_ROOT\Software\Java\target\myjar.jar
            %
            % GETSOURCEFOLDER
        ...
        function setter(obj,src,~,jObj)
            ...
            if ~ isempty(jObj)
                try
                    ...
                    v = obj.(src.Name);
                    if isenum(v)
                        % NOTE we have to cast the enum to a type that Java
                        % will understand. In most cases enums are chars
                        % but in case we have subclassed from a numeric or
                        % logical we dont want to cast to a char.
                        ...
                        elseif isnumeric(v)
                            ...
                            % We dont have any enums inherited from numeric
                            % types
                            v = double(v);
                        end
                    end
